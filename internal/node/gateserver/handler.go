...
func forwardToActor(app cfacade.IApplication, kind targetKind, method string,
	...
	if !ok {
		...
		clog.Warn("[gateserver] resolve target failed: kind=%d, method=%s", kind, method)
		return
	}
...
func afterEnterServer(appRaw interface{}, req *mynet.WsMsgReq, rsp *mynet.WsMsgRsp) {
	...
	if errCode != code.OK {
		clog.Warn("[gateserver] auto chatserver login failed: rid=%d, errCode=%d", rid, errCode)
	}
}
...
func makeOnConnClose(app cfacade.IApplication) func(mynet.WSConn) {
	return func(conn mynet.WSConn) {
		...
		if errCode != 0 {
			clog.Warn("[gateserver] notify sessionClose failed: uid=%d, errCode=%d", uid, errCode)
		}
	}
}
