...
func (this *ArmyLogic) init() {
	...
	for _, army := range armys {
		if army.Cmd != model.ArmyCmdIdle {
			e := army.End.Unix()
			if _, ok := this.endTimeArmys[e]; !ok {
	...
	for kT, armys := range this.endTimeArmys {
		if kT <= curTime {
			for _, a := range armys {
				switch a.Cmd {
				case model.ArmyCmdAttack, model.ArmyCmdDefend:
					this.Arrive(a)
...
func (this *ArmyLogic) exeUpdate(army *model.Army) {
	...
	if army.Cmd == model.ArmyCmdBack {
		...
		if armys, ok := this.stopInPosArmys[posId]; ok {
...
func (this *ArmyLogic) exeArrive(army *model.Army) {
	...
	case model.ArmyCmdAttack:
		if check.IsCanArrive(army.ToX, army.ToY, army.RId) &&
			!check.IsWarFree(army.ToX, army.ToY) &&
			!check.IsCanDefend(army.ToX, army.ToY, army.RId) {
	...
	case model.ArmyCmdDefend:
		if check.IsCanDefend(army.ToX, army.ToY, army.RId) {
	...
	case model.ArmyCmdReclamation:
		if army.State == model.ArmyRunning {
			if mgr.RBMgr.BuildIsRId(army.ToX, army.ToY, army.RId) {
				this.addStopArmy(army)
			...
			this.ArmyBack(army)
			if rr, ok := mgr.RResMgr.Get(army.RId); ok {
				if b, ok1 := mgr.RBMgr.PositionBuild(army.ToX, army.ToY); ok1 {
					rr.Stone += b.Stone
					rr.Iron += b.Iron
					rr.Wood += b.Wood
					// 与原 slgserver 保持一致：金币/粮食按既有数额翻倍
					rr.Gold += rr.Gold
					rr.Grain += rr.Grain
					rr.SyncExecute()
				}
...
func (this *ArmyLogic) ScanBlock(rid, x, y, length int) []*model.Army {
	...
	for i := x; i <= maxX; i++ {
		for j := y; j <= maxY; j++ {
			...
			if armys, ok := this.passByPosArmys[posId]; ok {
				if !ArmyIsInView(rid, i, j) {
					continue
				}
				for _, army := range armys {
					out = append(out, army)
...
func (this *ArmyLogic) Arrive(army *model.Army) { this.arriveArmys <- army }
func (this *ArmyLogic) Update(army *model.Army) { this.updateArmys <- army }
func (this *ArmyLogic) Interrupt(posId int)     { this.interruptId <- posId }
func (this *ArmyLogic) GiveUp(posId int)        { this.giveUpId <- posId }
...
func (this *ArmyLogic) addStopArmy(army *model.Army) {
	posId := global.ToPosition(army.ToX, army.ToY)
	this.stop.Lock()
	if _, ok := this.stopInPosArmys[posId]; !ok {
...
func (this *ArmyLogic) addAction(t int64, army *model.Army) {
	...
	if _, ok := this.endTimeArmys[t]; !ok {
...
func (this *ArmyLogic) PushAction(army *model.Army) {
	switch army.Cmd {
	case model.ArmyCmdAttack, model.ArmyCmdDefend, model.ArmyCmdTransfer:
		t := army.End.Unix()
...
func (this *ArmyLogic) GetSysArmy(x, y int) []*model.Army { return this.sys.GetArmy(x, y) }
func (this *ArmyLogic) DelSysArmy(x, y int)               { this.sys.DelArmy(x, y) }
