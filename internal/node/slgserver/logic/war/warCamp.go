...
func (this *warCamp) init() {
	...
	if this.attack.CityId > 0 {
		adds := mgr.RFMgr.GetAdditions(this.attack.CityId,
			facility.TypeForce,
	...
	if this.defense.CityId > 0 {
		adds := mgr.RFMgr.GetAdditions(this.defense.CityId,
			facility.TypeForce,
			facility.TypeDefense,
			facility.TypeSpeed,
			facility.TypeStrategy)
		for i, v := range adds {
			if i < len(defenseAdds) {
				defenseAdds[i] = v
			}
		}
	}
	...
	if aCamp > 0 {
		adds := mgr.RFMgr.GetAdditions(this.attack.CityId, facility.TypeHanAddition-1+int8(aCamp))
		if len(adds) > 0 {
			aCampAdds[0] = adds[0]
		}
	}
	...
	dCampAdds := []int{0}
	dCamp := this.defense.GetCamp()
	if dCamp > 0 {
		adds := mgr.RFMgr.GetAdditions(this.defense.CityId, facility.TypeHanAddition-1+int8(dCamp))
		if len(adds) > 0 {
			dCampAdds[0] = adds[0]
		}
	}
