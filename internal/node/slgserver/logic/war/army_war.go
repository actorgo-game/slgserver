...
// IArmyLogic 与原 ILogic.IArmyLogic 同义。放在 war 包内避免循环依赖。
...
func NewEmptyWar(attack *model.Army) *model.WarReport {
	//战报处理
	pArmy := attack.ToProto().(protocol.Army)
...
// 简单战斗
func NewBattle(attackArmy *model.Army, armyLogic IArmyLogic) *Battle {
	battle := Battle{
...
func (this *Battle) run() {
	...
	if ok {
		...
		if len(enemys) == 0 {
			...
			if lastWar != nil && lastWar.result > 1 {
...
func (this *Battle) trigger(army *model.Army, enemys []*model.Army, isRoleEnemy bool) (*warResult, []*model.WarReport) {
	...
	for _, enemy := range enemys {
		//战报处理
		pArmy := army.ToProto().(protocol.Army)
		pEnemy := enemy.ToProto().(protocol.Army)
...
func (this *Battle) OccupyRoleBuild(rid, x, y int) {
	...
	if b, ok := mgr.RBMgr.PositionBuild(x, y); ok {
		...
		oldId := b.RId
		clog.Info("hit in role build oldRId=%d newRId=%d", oldId, newId)
		b.RId = rid
	}
}
